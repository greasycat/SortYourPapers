...
impl TerminalBackend for DebugReviewBackend {
    ...
    fn prompt_inspect_review_action(
    ...
    ) -> crate::terminal::Result<InspectReviewPrompt> {
        *self.inspect_calls.lock().expect("inspect calls") += 1;
        self.inspect_replies
            ...
            .pop_front()
            .ok_or_else(|| {
                crate::terminal::AppError::Execution("missing debug inspect reply".to_string())
            })
    }

    fn prompt_continue_improving(&self) -> crate::terminal::Result<bool> {
        *self.continue_calls.lock().expect("continue calls") += 1;
        self.continue_replies
            ...
            .pop_front()
            .ok_or_else(|| {
                crate::terminal::AppError::Execution("missing debug continue reply".to_string())
            })
    }
}
