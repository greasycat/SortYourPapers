...
use syp_library::error::AppError as LibraryError;

use crate::{
    config::AppConfig,
    error::{AppError, Result},
    llm::{self, ChatConfig, build_client},
    papers::extract::{ExtractorMode, extract_text_batch, reset_debug_extract_log},
    papers::placement::PlacementDecision,
    papers::taxonomy::{
        KeywordBatchProgress, ReferenceEmbeddingOptions, ReferenceEvidenceOptions,
        TaxonomyBatchProgress, collect_reference_evidence, extract_keywords_with_progress,
...
fn build_llm_client_stage(
    ...
    let client = Arc::<dyn llm::LlmClient>::from(build_client(&ChatConfig {
...
async fn extract_keywords_stage(
    ...
    let (keyword_state, usage) = extract_keywords_with_progress(
        ...
        |progress| {
            ...
            workspace
                .save_artifact(KEYWORD_BATCH_PROGRESS_FILE, progress)
                .map_err(|err| LibraryError::Execution(err.to_string()))?;
            workspace
                .save_report(report)
                .map_err(|err| LibraryError::Execution(err.to_string()))
        },
...
async fn synthesize_categories_stage(
    ...
    let batch_progress = synthesize_category_batches_with_progress(
        ...
        |progress| {
            ...
            workspace
                .save_artifact(TAXONOMY_BATCH_PROGRESS_FILE, progress)
                .map_err(|err| LibraryError::Execution(err.to_string()))?;
            workspace
                .save_report(report)
                .map_err(|err| LibraryError::Execution(err.to_string()))
    ...
    let partial_categories = batch_progress
        ...
        .collect::<Vec<_>>();
    let (reference_evidence, reference_usage) = collect_reference_evidence(
        papers,
        &ReferenceEvidenceOptions {
            assistance: config.taxonomy_assistance,
            reference_manifest_path: config.reference_manifest_path.clone(),
            reference_top_k: config.reference_top_k,
            embedding: ReferenceEmbeddingOptions {
