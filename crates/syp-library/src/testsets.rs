...
const DEV_CONFIG: &str = "dev.toml";
const TESTSETS_DIR: &str = "testsets";
pub const DEFAULT_MANIFEST_PATH: &str = "assets/testsets/scijudgebench-diverse.toml";
...
pub fn shared_cache_root() -> Result<PathBuf> {
    if let Ok(current_dir) = env::current_dir()
        && let Some(path) = shared_testset_cache_dir_from(&current_dir)?
    {
        return Ok(path);
    }
...
fn shared_testset_cache_dir_from(start: &Path) -> Result<Option<PathBuf>> {
    let Some(dev_path) = find_ancestor_file(start, DEV_CONFIG) else {
        return Ok(None);
    };
    let raw = fs::read_to_string(&dev_path)?;
    let dev_config: DevConfig = toml::from_str(&raw)?;
    let relative = match dev_config.testsets.cache_dir {
        Some(path) => path,
        None => {
    ...
    let root = dev_path.parent().ok_or_else(|| {
        AppError::Execution(format!("invalid dev config path {}", dev_path.display()))
    })?;
...
fn default_testset_cache_dir() -> Result<PathBuf> {
    let dirs = directories::ProjectDirs::from("org", "sortyourpapers", "sortyourpapers")
        .ok_or_else(|| AppError::Config("could not resolve XDG cache directory".to_string()))?;
    Ok(dirs.cache_dir().join(TESTSETS_DIR))
}
