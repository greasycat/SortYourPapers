...
fn contains_symbol_with_fg(app: &App, width: u16, height: u16, symbol: &str, fg: Color) -> bool {
    let terminal = render_app(app, width, height);
    let buffer = terminal.backend().buffer();
    let area = buffer.area();
...
fn run_form_can_be_hydrated_from_saved_config() {
    let config = AppConfig {
        ...
        taxonomy_assistance: crate::papers::taxonomy::TaxonomyAssistance::LlmOnly,
        ...
        embedding_provider: crate::llm::LlmProvider::Openai,
        embedding_model: "text-embedding-3-small".to_string(),
        embedding_base_url: Some("http://localhost:1234/embeddings".to_string()),
        embedding_api_key: Some(ApiKeySource::Env("OPENAI_API_KEY".to_string())),
...
fn run_form_stacks_side_panels_on_narrow_width() {
    ...
    let (_, preview_y) =
        find_text_position(&lines, "Launch Preview").expect("launch preview panel should render");
    let (_, selected_y) =
        find_text_position(&lines, "Selected Field").expect("selected field panel should render");
...
fn operation_screen_renders_tabs_and_success_actions() {
    ...
    assert!(
        lines
            .iter()
            .any(|line| line.contains("Next actions: 3 Taxonomy, 4 Planned Actions, s Sessions."))
    );
}
...
fn taxonomy_review_submits_suggestion_and_enters_waiting_phase() {
    ...
    for character in "Merge vision".chars() {
        runtime
            .block_on(app.handle_key(KeyEvent::new(KeyCode::Char(character), KeyModifiers::NONE)))
...
fn operation_header_wraps_shortcuts_on_narrow_width() {
    ...
    let (_, hint_y) =
        find_text_position(&lines, "Tab/h/l: switch").expect("wrapped shortcut hint should render");
