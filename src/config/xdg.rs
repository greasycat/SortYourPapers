...
use crate::{
    config::{
        ...
        DEFAULT_USE_CURRENT_FOLDER_TREE, FileConfig, TuiPreferences,
    },
...
pub(super) fn load_tui_preferences() -> Result<TuiPreferences> {
...
pub(super) fn save_tui_preferences(prefs: &TuiPreferences) -> Result<PathBuf> {
    let Some(path) = xdg_tui_preferences_path() else {
        return Err(AppError::Config(
            "could not resolve XDG config directory".to_string(),
        ));
...
pub(super) fn load_tui_preferences_from_path(path: &Path) -> Result<TuiPreferences> {
    if !path.exists() {
        return Ok(TuiPreferences::default());
    }
...
pub(super) fn write_tui_preferences_at(path: &Path, prefs: &TuiPreferences) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }

    let serialized = toml::to_string(prefs)
        .map_err(|err| AppError::Config(format!("failed to serialize tui prefs: {err}")))?;
    fs::write(
        path,
        format!(
